...
class Sample_BoundingBox {
  ...
  async run() {
    ...
    // Engine3D.getRenderJob(this.view) // 이 코드가 왜 있는거지?

    this.box = Object3DUtil.GetSingleCube(5, 3, 8, 1, 1, 1);
    this.box.transform.eventDispatcher.addEventListener(
  ...
  logChange() {
    console.log("BoundingBox changed");
  }
  ...
  loop() {
    this.view.graphic3D.drawBoundingBox(
      this.box.instanceID,
      this.box.bound as any,
      this.green
    );
    this.view.graphic3D.drawBoundingBox(
      this.container.instanceID,
      this.container.bound as any,
      this.red
    );
  }
}
