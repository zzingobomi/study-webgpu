...
class Sample_MeshLines {
  ...
  async run() {
    ...
    f.addColor(
      {
        lineColor: Object.values(this.lineColor).map((v, i) =>
          i === 3 ? v : v * 255
        ),
      },
      "lineColor"
    ...
    f.add(
      { resetView: () => this.hoverCameraController.setCamera(0, 0, 20) },
      "resetView"
    );
    f.add(
      {
  ...
  drawPoint(x: number, y: number) {
    ...
    this.camera.worldToScreenPoint(
      this.hoverCameraController.target,
      Vector3.HELP_0
    );
    const pos = this.camera.screenPointToWorld(
      x,
      y,
      Vector3.HELP_0.z + this.depth / 100
    );
    point.x = pos.x;
    point.y = pos.y;
    point.z = pos.z;
  ...
  drawLine(x: number, y: number) {
    ...
    const start = this.camera.screenPointToWorld(
      this.lastX,
      this.lastY,
      Vector3.HELP_0.z + this.depth / 100
    );
