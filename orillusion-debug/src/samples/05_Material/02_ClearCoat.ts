...
class Sample_ClearCoat {
  ...
  async initScene() {
    /******** sky *******/
    {
      ...
      sky.map = await Engine3D.res.loadHDRTextureCube(
        "https://cdn.orillusion.com//hdri/sunset.hdr"
      );
    ...
    /******** light *******/
    {
      ...
      DirLight.addColor(
        {
          lightColor: [
            directLight.lightColor.r,
            directLight.lightColor.g,
            directLight.lightColor.b,
            1,
          ].map((v, i) => (i == 3 ? v : Math.floor(v * 255))),
        },
        "lightColor"
      ).onChange((v) => {
        directLight.lightColor = new Color(
          v[0] / 255,
          v[1] / 255,
          v[2] / 255,
          v[3]
    ...
    {
      let clearCoatRoughnessTex = await Engine3D.res.loadTexture(
        "https://cdn.orillusion.com/PBR/ClearCoatTest/T_Imperfections_Wipe_Mask.PNG"
      );
      let space = 50;
