...
export class WasmMatrix {
    ...
    await new Promise((resolve) => {
      ...
      script.onload = () => {
        let check = () => {
          ...
          if (this.wasm && this.wasm["calledRun"]) resolve(true);
  ...
  public static allocMatrix(count: number) {
    ...
    this.matrixBuffer = new Float32Array(
      this.wasm.HEAPF32.buffer,
      this.matrixBufferPtr,
      16 * count
    );
    this.matrixSRTBuffer = new Float32Array(
      this.wasm.HEAPF32.buffer,
      this.matrixSRTBufferPtr,
      3 * 3 * count
    );
    this.matrixContinuedSRTBuffer = new Float32Array(
      this.wasm.HEAPF32.buffer,
