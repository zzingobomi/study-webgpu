...
export class StringUtil {
   ...
   */
  public static getEllipsis(str: any, len: number = 4): string {
    let name = str;
   ...
   * @returns result string
   */
  static strCut(str: string, len: number): string {
    if (str.length * 2 <= len) {
      return str;
    }
    var strlen = 0;
   ...
   * @returns result string
   */
  static stringFormat(str: string, ...params: string[]): string {
   ...
   * @returns result string
   */
  static parseJson2String(
    json: string,
    options?: {
      newlineAfterColonIfBeforeBraceOrBracket?: any;
      spaceAfterColon?: any;
    }
  ): string {
    let reg = null,
   ...
   * @returns hash code
   */
  public static stringToHash(str: string) {
    let hash = 0;
