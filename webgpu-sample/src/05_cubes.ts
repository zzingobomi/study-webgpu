...
async function initPipeline(
  ...
  // create a uniform group for Maxtrix1
  ...
  // create a uniform group for Maxtrix2
...
async function run() {
  ...
  function frame() {
    ...
    const now = Date.now() / 1000;
    {
      ...
      device.queue.writeBuffer(pipelineObj.mvpBuffer1, 0, mvpMatrix1);
    }
    {
      rotation2.x = Math.cos(now);
      rotation2.y = Math.sin(now);
      const mvpMatrix2 = getMvpMatrix(aspect, position2, rotation2, scale2);
      device.queue.writeBuffer(pipelineObj.mvpBuffer2, 0, mvpMatrix2);
    }
