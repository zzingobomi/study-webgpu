import shadowVertex from "./shaders/shadow.vert.wgsl?raw";
...
async function initPipeline(
  ...
  const modelViewBuffer = device.createBuffer({
    label: "GPUBuffer store n*4*4 matrix",
...
async function run() {
  ...
  for (let i = 2; i < NUM; i++) {
    ...
    const position = {
      x: (1 + Math.random() * 12) * or,
      y: -8 + Math.random() * 15,
      z: -20 + (1 + Math.random() * 12) * or,
    };
    ...
    scene.push({
      ...
      scale,
      y: position.y,
      v: Math.max(0.09, Math.random() / 10) * or,
    });
  }
  ...
  function frame() {
    ...
    device.queue.writeBuffer(
      pipelineObj.lightProjectionBuffer,
      0,
      lightProjectionMatrix as Float32Array
    );
    device.queue.writeBuffer(
      pipelineObj.lightBuffer,
      0,
    ...
    for (let i = 2; i < NUM; i++) {
      ...
      const modelView = getModelViewMatrix(
        obj.position,
        obj.rotation,
        obj.scale
      );
  ...
  function updateCamera() {
    ...
    device.queue.writeBuffer(
      pipelineObj.cameraProjectionBuffer,
      0,
      projectionMatrix
    );
  }
